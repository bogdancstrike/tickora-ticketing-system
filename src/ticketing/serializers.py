...
def serialize_ticket(t: Ticket, p: Principal, *, full: bool = True) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "subcategory_code": getattr(t, "subcategory_code", None),
        "subcategory_name": getattr(t, "subcategory_name", None),
        "beneficiary_type": t.beneficiary_type,
        "title":            t.title,
        "current_sector_code": sector_code,
        "beneficiary_user_id": getattr(t, "beneficiary_user_id", None),
        # Multi-assignment lists hydrated by the service.
        "sector_codes":     list(getattr(t, "sector_codes", []) or []),
        "assignee_user_ids": [uid for uid, _ in getattr(t, "assignee_users", []) or []],
        "assignee_usernames": [uname for _, uname in getattr(t, "assignee_users", []) or []],
